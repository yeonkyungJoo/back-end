...
//        study.addMember(범석);
    ...
    @Test
    void 스터디원_삭제(){
        study.removeMember(세로);
        entityManager.flush();
        List<Account> member=study.getMember();
        Assertions.assertFalse(member.contains(세로));
    }
    @Test
    void 관리자_리스트(){
        List<Account> manager=study.getManager();
        Assertions.assertTrue(manager.contains(근우));
    }
}
